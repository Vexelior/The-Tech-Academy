# Person Information Tracking
#
# Python Ver:   3.9.1
#
# Author:       Alexander W. Sanderson
#
# Purpose:      A mock Person Information Application using Python, Tkinter, and SQLite.
#
...
def load_gui(self):
    # These are our labels for outputs and inputs.
    ...
    self.lbl_course = tk.Label(self.master,text='Street Address:')
    ...
    self.btn_add = tk.Button(self.master,width=12,height=2,text='Add',command=lambda: func.addToList(self))
    self.btn_add.grid(row=10,column=0,padx=(25,0),pady=(45,10),sticky=W)
    self.btn_update = tk.Button(self.master,width=12,height=2,text='Update',command=lambda: func.onUpdate(self))
    self.btn_update.grid(row=10,column=1,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: func.onDelete(self))
    self.btn_delete.grid(row=10,column=2,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_close = tk.Button(self.master,width=12,height=2,text='Close',command=lambda: func.ask_quit(self))
    self.btn_close.grid(row=10,column=4,columnspan=1,padx=(15,0),pady=(45,10),sticky=E)

    func.create_db(self) # Creates the database if it does not exist previously.
    func.onRefresh(self)
